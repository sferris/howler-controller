...
func Mode(mode string) (Modes, bool) {
  switch strings.ToLower(mode) {
    // 1 (0x01)
    case "1": fallthrough
    case "joystick1": fallthrough
    case "modejoystick1":
      return ModeJoystick1, true

    // 2 (0x02)
    case "2": fallthrough
    case "joystick2": fallthrough
    case "modejoystick2":
      return ModeJoystick2, true
...
func (howler *HowlerDevice) SetInput(input Inputs, mode Modes, value InputValues, 
  modifier Modifiers) (HowlerInput, error) {
  var stmt = []byte{HowlerID,0x03,byte(input),byte(mode),byte(value),byte(modifier),
